/* SPDX-License-Identifier: BSD-1-Clause (FatFs) */
/*
 * tclwext4 configuration for FatFs.
 *
 * FatFs is configured by editing this file - ff.h pulls it in with a quoted
 * include, which MSVC resolves against ff.h's own directory first, so there is
 * no way to override it from the command line the way lwext4 allows. To keep
 * external/fatfs pristine, the build copies the FatFs sources into the build
 * tree and drops this file alongside them.
 *
 * Changes from upstream defaults:
 *   FF_USE_LFN 3        long filenames (heap buffer) - an ESP is full of them
 *   FF_LFN_UNICODE 1    UTF-16 API, matching the WFX W-API with no conversion.
 *                       (1 is UTF-16; 2 is UTF-8. With 2, ff.h typedefs TCHAR as
 *                       char, which collides with the wchar_t TCHAR from
 *                       windows.h that ff.h itself pulls in on _WIN32.)
 ...
 *   FF_FS_EXFAT 1       exFAT, occasionally used for large data partitions
 *   FF_USE_CHMOD 1      f_chmod / f_utime - needed by FsSetAttrW / FsSetTimeW
 */
/*---------------------------------------------------------------------------/
